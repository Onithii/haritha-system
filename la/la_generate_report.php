...
// 1. Secure Access Check: Configured Role ID to 3 (Local Authority) to ensure decoupled access permissions
if (!isset($_SESSION['role_id']) || $_SESSION['role_id'] != 3 || empty($_SESSION['user_id'])) {
    session_unset();
    session_destroy();
...
$la_user_id = (int)$_SESSION['user_id'];

// Fetch logged-in LA profile info to display context on the report sheet securely using prepared statements
$la_profile = ['f_name' => 'Authority', 'l_name' => 'Officer', 'district' => 'Your District'];
$la_stmt = mysqli_prepare($conn, "SELECT f_name, l_name, district FROM users WHERE user_id = ? LIMIT 1");
if ($la_stmt) {
    mysqli_stmt_bind_param($la_stmt, "i", $la_user_id);
    mysqli_stmt_execute($la_stmt);
    $la_result = mysqli_stmt_get_result($la_stmt);
    if ($row = mysqli_fetch_assoc($la_result)) {
        $la_profile = $row;
    }
    mysqli_stmt_close($la_stmt);
}

$la_district = mysqli_real_escape_string($conn, $la_profile['district']);
...
if (!empty($report_type) && $report_type == 'complaint_summary') {
    ...
    // Encapsulate queries strictly inside the LA Regional Jurisdiction and the time framework
    $where_str = "c.district = '$la_district' AND c.created_at BETWEEN '$start_date 00:00:00' AND '$end_date 23:59:59'";

    // Fetch total matching cases assigned to this regional District
    ...
    // Fetch detailed regional records strictly matching scope limits
...
<head>
    <meta charset="UTF-8">
    <title>LA Environmental Analytics Engine</title>
    <style>
        body { font-family: Arial, sans-serif; background-color: #f2f5f2; margin: 0; padding-bottom: 60px; color: #333; }
        .header { background-color: #1565c0; color: white; padding: 25px; text-align: center; position: relative; }
        .btn-back { position: absolute; left: 20px; top: 30px; background-color: white; color: #1565c0; padding: 8px 15px; border-radius: 5px; text-decoration: none; font-weight: bold; font-size: 14px; }
        ...
        .filter-group label { font-weight: bold; margin-bottom: 5px; font-size: 14px; color: #1565c0; }
        ...
        .btn-print { background-color: #00897b; color: white; }
        .btn-print:hover { background-color: #004d40; }

        .report-paper { background: white; padding: 40px; border-radius: 8px; box-shadow: 0px 4px 12px rgba(0,0,0,0.1); border-top: 8px solid #1565c0; }
        .report-title-block { text-align: center; border-bottom: 2px dashed #1565c0; padding-bottom: 20px; margin-bottom: 25px; }
        .report-title-block h2 { margin: 0; color: #1565c0; text-transform: uppercase; letter-spacing: 1px; }
        ...
        .metric-box { background: #e3f2fd; padding: 15px; border-radius: 6px; text-align: center; border-left: 4px solid #1565c0; }
        ...
        .breakdown-card h4 { margin-top: 0; border-bottom: 1px solid #ccc; padding-bottom: 5px; color: #1565c0; }
        ...
        .data-table th { background-color: #f5f5f5; color: #1565c0; font-weight: bold; }
...
<div class="header">
    <a href="la_dash.php" class="btn-back">← Back to Panel</a>
    <h1>Haritha Authority Performance Engine</h1>
    <p>Regional Environmental Tracking & Aggregated Performance Panel</p>
</div>
    ...
    <div class="filter-card">
        <form method="GET" action="">
            <div class="filter-grid">
                <div class="filter-group">
                    ...
                    <select name="report_type" id="report_type" required>
                        ...
                        <option value="complaint_summary" <?php if($report_type == 'complaint_summary') echo 'selected'; ?>>Regional Environmental Complaint Summary Report</option>
                    </select>
                </div>
            ...
            <div class="action-bar">
                ...
                <?php endif; ?>
                <a href="la_generate_report.php" class="btn btn-secondary">Reset Filters</a>
                <button type="submit" class="btn btn-primary">Compile District Report</button>
            </div>
        </form>
    </div>
    ...
    <?php if($report_generated): ?>
        <div class="report-paper">
            ...
            <div class="report-title-block">
                <h2>Haritha Regional Environmental Complaint Report</h2>
                <div class="report-meta">
                    <strong>Target Period:</strong> <?php echo htmlspecialchars($start_date); ?> to <?php echo htmlspecialchars($end_date); ?><br>
                    <strong>Officer In-Charge:</strong> Authority Officer <?php echo htmlspecialchars($la_profile['f_name'] . ' ' . $la_profile['l_name']); ?> (ID: #<?php echo $la_user_id; ?>)<br>
                    <strong>Assigned Jurisdiction:</strong> District-wide Coverage: <?php echo htmlspecialchars($la_profile['district'] ?? 'Not Specified'); ?>
                    <br>
                    <span style="font-size: 11px; color:#888;">System Generated on: <?php echo date('Y-m-d H:i:s'); ?> | Local Authority Secure View Mode</span>
                </div>
            </div>
            ...
            <div class="metrics-grid">
                <div class="metric-box">
                    <div class="metric-label">Total Regional Claims</div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <?php endwhile; else: ?>
                        <tr><td colspan="6" style="text-align:center; color:#999;">No records found matching your regional context inside the selected dates.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
