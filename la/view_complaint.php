<?php
// 1. Enable Full Diagnostic Logs to instantly catch missing variables or table errors
...
// 2. Secure Access Check: Ensure user is logged in as a Local Authority Official (Role ID: 3) or Admin (Role ID: 5)
if (!isset($_SESSION['role_id']) || !in_array($_SESSION['role_id'], [3, 5]) || empty($_SESSION['user_id'])) {
    session_unset();
    session_destroy();
...
if ($complaint_id === 0) {
    die("<div style='font-family:sans-serif; padding:20px; color:#c62828;'><b>Error:</b> Invalid or missing Complaint ID.</div>");
}
...
if (isset($_GET['action']) && $_GET['action'] === 'get_nearest_official' && isset($_GET['role_level'])) {
    ...
    $role_level = intval($_GET['role_level']);
    $comp_lat = isset($_GET['lat']) ? floatval($_GET['lat']) : 0.0;
    $comp_lng = isset($_GET['lng']) ? floatval($_GET['lng']) : 0.0;
...
// 3. Handle Form Submissions (Update and Escalation Actions)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_type'])) {
    if ($_POST['action_type'] === 'update_complaint') {
        $new_status_id = intval($_POST['status_id']);
        $reply_text = trim($_POST['reply_text']);
        
        $update_q = "UPDATE complaints SET status_id = ?, reply = ?, updated_at = NOW() WHERE complaint_id = ?";
        $stmt = mysqli_prepare($conn, $update_q);
        mysqli_stmt_bind_param($stmt, "isi", $new_status_id, $reply_text, $complaint_id);
        
        if (mysqli_stmt_execute($stmt)) {
            $message = "<div class='alert success'>Complaint updated successfully.</div>";
        } else {
            $message = "<div class='alert error'>Error updating database configurations: " . mysqli_error($conn) . "</div>";
        }
    ...
    if ($_POST['action_type'] === 'escalate_complaint') {
        ...
        if ($target_officer_id > 0) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $message = "<div class='alert error'>Failed to process assignment allocation.</div>";
                mysqli_stmt_close($stmt);
            }
        } else {
...
// 4. Fetch complaint information safely
...
if (!$complaint) {
    die("<div style='font-family:sans-serif; padding:40px; text-align:center;'><h2>Complaint Records Not Found</h2><p>The requested complaint entry does not exist in the system core database.</p><br><a href='la_dash.php'>Return to Dashboard</a></div>");
}
...
<head>
    <meta charset="UTF-8">
    <title>LA Investigation Desk — Complaint #<?php echo $complaint['complaint_id']; ?></title>
    <style>
        body { font-family: Arial, sans-serif; background-color: #f2f5f2; margin: 0; padding-bottom: 60px; color: #333; }
        .header { background-color: #006064; color: white; padding: 15px 30px; display: flex; justify-content: space-between; align-items: center; }
        .back-btn { background: #00363a; color: white; text-decoration: none; padding: 8px 15px; border-radius: 5px; font-weight: bold; font-size: 14px; }
        .back-btn:hover { background: #001214; }
        .wrapper { width: 85%; margin: 30px auto; display: flex; gap: 30px; }
        .left-panel { width: 60%; }
        .right-panel { width: 40%; display: flex; flex-direction: column; gap: 20px; }
        .content-box { background: white; padding: 25px; border-radius: 10px; box-shadow: 0px 2px 8px rgba(0,0,0,0.1); margin-bottom: 25px; }
        h2, h3 { color: #006064; margin-top: 0; border-bottom: 2px solid #b2dfdb; padding-bottom: 8px; }
        ...
        button { background-color: #006064; color: white; border: none; padding: 12px 20px; cursor: pointer; border-radius: 5px; width: 100%; font-weight: bold; font-size: 14px; margin-top: 15px; text-transform: uppercase; }
        ...
        .radio-option { display: flex; align-items: center; gap: 5px; font-size: 13px; font-weight: bold; cursor: pointer; color: #006064; }
...
<div class="header">
    <h1>Haritha LA Jurisdiction Desk</h1>
    <a href="la_dash.php" class="back-btn">← Back to Dashboard</a>
</div>
...
<div class="wrapper">
    <div class="left-panel">
        ...
        <div class="content-box">
            ...
            <div class="meta-grid">
                ...
                <div class="meta-item"><b>Coordinates:</b> <?php echo $comp_lat . ", " . $comp_lng; ?></div>
            </div>
    ...
    <div class="right-panel">
        ...
        <div class="content-box">
            <h3>Escalate / Reallocate Case</h3>
            <form method="POST" action="">
                ...
                <div class="radio-group">
                    ...
                    <label class="radio-option"><input type="radio" name="authority_level" value="3" onclick="fetchNearestOfficial(3)"> Local Authority (Other)</label>
                ...
                <button type="submit" style="background-color: #b71c1c;">Confirm Reallocation</button>
            </form>
        </div>
    </div>
</div>
...
function fetchNearestOfficial(roleLevel) {
    ...
    displayBox.value = "Locating nearest options via spatial data...";
    distanceBox.innerText = "";
    targetIdInput.value = "";

    let url = `view_complaint.php?id=<?php echo $complaint_id; ?>&action=get_nearest_official&role_level=${roleLevel}&lat=<?php echo $comp_lat; ?>&lng=<?php echo $comp_lng; ?>`;
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success) {
                ...
                distanceBox.innerText = `📍 Closest choice located approximately ${data.distance} km away from incident site.`;
            } else {
                displayBox.value = data.message || "No official record found for this tier level.";
                distanceBox.innerText = "";
            }
        })
        .catch(err => {
