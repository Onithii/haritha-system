...
// 3. Fetch Event Metadata to confirm ownership scope
...
if ($evt_stmt) {
    mysqli_stmt_bind_param($evt_stmt, "ii", $event_id, $la_id);
    ...
    if ($row = mysqli_fetch_assoc($evt_result)) {
        ...
        // Force redirect if event exists but wasn't deployed by this explicit LA authority account
        header("Location: participation_manage.php");
        exit();
    }
...
// 4. Handle Certificate Processing Submission
$msg = "";
$msg_class = "";
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['issue_certificates'])) {
    if (!empty($_POST['certified_users'])) {
        ...
        try {
            $success_count = 0;
            $update_query = "UPDATE volunteer_participants SET certificate_issued = 1 WHERE event_id = ? AND user_id = ?";
            $up_stmt = mysqli_prepare($conn, $update_query);
            
            if ($up_stmt) {
                foreach ($selected_users as $u_id) {
                    $target_user = intval($u_id);
                    mysqli_stmt_bind_param($up_stmt, "ii", $event_id, $target_user);
            ...
            mysqli_commit($conn);
            $msg = "Success: Validation logs processed and certificates issued to " . $success_count . " verified participant(s).";
            $msg_class = "msg-success";
        } catch (Exception $e) {
            mysqli_rollback($conn);
            $msg = "Error: System failed to batch generate certificate records.";
            $msg_class = "msg-error";
        }
    } else {
...
$participants = [];
$participants_query = "SELECT u.user_id, u.name, u.email, vp.attendance_verified, vp.certificate_issued 
...
<head>
    <meta charset="UTF-8">
    <title>View Participants - Local Authority</title>
    <style>
        ...
        .table-section h2 { color: #006064; margin-top: 0; padding-bottom: 5px; margin-bottom: 5px; }
        .subtitle { color: #757575; font-style: italic; margin-bottom: 20px; border-bottom: 2px solid #b2dfdb; padding-bottom: 10px; }
        ...
        /* Certificate Actions */
        .cert-link { color: #006064; font-weight: bold; text-decoration: none; font-size: 14px; }
        .cert-link:hover { text-decoration: underline; color: #00363a; }
        .cert-disabled { color: #9e9e9e; font-style: italic; font-size: 13px; }
        .issued-tag { color: #2e7d32; font-size: 13px; font-weight: bold; }

        /* Action Layout Footer */
        .action-footer { margin-top: 25px; padding-top: 15px; border-top: 1px solid #ddd; text-align: right; }
        .btn-submit { background-color: #006064; color: white; border: none; padding: 12px 25px; cursor: pointer; border-radius: 5px; font-weight: bold; font-size: 14px; }
...
<div class="table-section">
    ...
    <div class="subtitle">Event ID Reference: #<?php echo $event_id; ?> | Local Authority Administrative Roster</div>
                ...
                <tbody>
                    <?php foreach ($participants as $citizen): ?>
                        <?php 
                            ...
                            $already_issued = (isset($citizen['certificate_issued']) && $citizen['certificate_issued'] == 1);
                        ?>
                        <tr>
                            <td>#<?php echo $citizen['user_id']; ?></td>
                            <td><b><?php echo htmlspecialchars($citizen['name']); ?></b></td>
                            ...
                            <td>
                                <?php if ($is_verified): ?>
                                    <?php if ($already_issued): ?>
                                        <span class="issued-tag">Issued</span>
                                    <?php else: ?>
