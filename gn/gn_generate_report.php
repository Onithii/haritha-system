...
// 1. Secure Access Check: Matched Role ID back to 2 (Grama Niladhari) to fix the login redirect loop
...
$gn_user_id = (int)$_SESSION['user_id'];

// Fetch logged-in GN profile info to display context on the report sheet securely using prepared statements
$gn_profile = ['f_name' => 'Officer', 'l_name' => '', 'gn_division' => 'Your Division', 'district' => 'Your District'];
$gn_stmt = mysqli_prepare($conn, "SELECT f_name, l_name, gn_division, district FROM users WHERE user_id = ? LIMIT 1");
if ($gn_stmt) {
    mysqli_stmt_bind_param($gn_stmt, "i", $gn_user_id);
    mysqli_stmt_execute($gn_stmt);
    $gn_result = mysqli_stmt_get_result($gn_stmt);
    if ($row = mysqli_fetch_assoc($gn_result)) {
        $gn_profile = $row;
    }
...
if (!empty($report_type) && $report_type == 'complaint_summary') {
    ...
    // Encapsulate queries strictly inside the assigned GN context and the time framework
    $where_str = "c.assigned_to_id = $gn_user_id AND c.created_at BETWEEN '$start_date 00:00:00' AND '$end_date 23:59:59'";

    // Fetch total matching cases assigned to this specific GN
...
<head>
    <meta charset="UTF-8">
    <title>GN Environmental Performance Desk</title>
    <style>
        body { font-family: Arial, sans-serif; background-color: #f2f5f2; margin: 0; padding-bottom: 60px; color: #333; }
        .header { background-color: #2e7d32; color: white; padding: 25px; text-align: center; position: relative; }
        .btn-back { position: absolute; left: 20px; top: 30px; background-color: white; color: #2e7d32; padding: 8px 15px; border-radius: 5px; text-decoration: none; font-weight: bold; font-size: 14px; }
        ...
        .report-paper { background: white; padding: 40px; border-radius: 8px; box-shadow: 0px 4px 12px rgba(0,0,0,0.1); border-top: 8px solid #2e7d32; }
        .report-title-block { text-align: center; border-bottom: 2px dashed #2e7d32; padding-bottom: 20px; margin-bottom: 25px; }
        .report-title-block h2 { margin: 0; color: #2e7d32; text-transform: uppercase; letter-spacing: 1px; }
        ...
        .breakdown-card h4 { margin-top: 0; border-bottom: 1px solid #ccc; padding-bottom: 5px; color: #2e7d32; }
        ...
        .data-table th { background-color: #f5f5f5; color: #2e7d32; font-weight: bold; }
    ...
    <div class="filter-card">
        <form method="GET" action="">
            ...
            <div class="action-bar">
                ...
                <?php endif; ?>
                <a href="gn_generate_report.php" class="btn btn-secondary">Reset Filters</a>
        ...
        <div class="report-paper">
            ...
            <div class="report-title-block">
                ...
                <div class="report-meta">
                    ...
                    <strong>Officer In-Charge:</strong> Officer <?php echo htmlspecialchars($gn_profile['f_name'] . ' ' . $gn_profile['l_name']); ?> (ID: #<?php echo $gn_user_id; ?>)<br>
                    <strong>Assigned Jurisdiction:</strong> GN Division: <?php echo htmlspecialchars($gn_profile['gn_division'] ?? 'Not Specified'); ?> | District: <?php echo htmlspecialchars($gn_profile['district'] ?? 'Not Specified'); ?>
                    <br>
