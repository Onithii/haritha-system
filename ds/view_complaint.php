...
// 2. Secure Access Check: Ensure user is logged in as a Divisional Secretariat (Role ID: 4)
...
$query = "SELECT c.*, cc.category_name, cs.status_name 
          ...
          WHERE c.complaint_id = ? LIMIT 1"; // Note: Removed strict ownership check temporarily to prevent blank pages during reassignment testing
...
if (!$complaint) {
    die("<div style='font-family:sans-serif; padding:40px; text-align:center;'><h2>Complaint Records Not Found</h2><p>The requested complaint entry does not exist in the system core database.</p><br><a href='ds_dash.php'>Return to Dashboard</a></div>");
}
...
<head>
    <meta charset="UTF-8">
    <title>DS Investigation Desk — Complaint #<?php echo $complaint['complaint_id']; ?></title>
    <style>
        body { font-family: Arial, sans-serif; background-color: #f2f5f2; margin: 0; padding-bottom: 60px; color: #333; }
        .header { background-color: #0d47a1; color: white; padding: 15px 30px; display: flex; justify-content: space-between; align-items: center; }
        .back-btn { background: #002171; color: white; text-decoration: none; padding: 8px 15px; border-radius: 5px; font-weight: bold; font-size: 14px; }
        .back-btn:hover { background: #0a2240; }
        ...
        h2, h3 { color: #0d47a1; margin-top: 0; border-bottom: 2px solid #bbdefb; padding-bottom: 8px; }
        ...
        button { background-color: #0d47a1; color: white; border: none; padding: 12px 20px; cursor: pointer; border-radius: 5px; width: 100%; font-weight: bold; font-size: 14px; margin-top: 15px; text-transform: uppercase; }
        ...
        .radio-option { display: flex; align-items: center; gap: 5px; font-size: 13px; font-weight: bold; cursor: pointer; color: #0d47a1; }
...
<div class="header">
    <h1>Haritha DS Jurisdiction Desk</h1>
        ...
        <div class="content-box">
            ...
            <form method="POST" action="">
                ...
                <div class="radio-group">
                    ...
                    <label class="radio-option"><input type="radio" name="authority_level" value="4" onclick="fetchNearestOfficial(4)"> DS (Other)</label>
                </div>
...
function fetchNearestOfficial(roleLevel) {
    ...
    // Appending coordinates straight through JS params to handle quick lookup processing flawlessly
