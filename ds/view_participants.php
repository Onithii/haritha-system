...
// 1. Secure Access Check: Ensure user is logged in and is a Divisional Secretariat (Role ID: 4)
...
// 3. Fetch Event Metadata to confirm ownership/creation bounds
...
if ($evt_stmt) {
    mysqli_stmt_bind_param($evt_stmt, "ii", $event_id, $ds_id);
    ...
    if ($row = mysqli_fetch_assoc($evt_result)) {
        ...
        // Fallback or exit if the campaign does not belong to this DS officer
        header("Location: participation_manage.php");
        exit();
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['issue_certificates'])) {
    if (!empty($_POST['certified_users'])) {
        ...
        /* Operational Note: Loop updates/logs state or handles logic.
           Can be replaced with standard database query updating an issue column 
        ...
        */
        ...
        $msg = "Success: Regional certificates processed and issued to $success_count verified participant(s).";
...
<head>
    ...
    <style>
        ...
        .table-section h2 { color: #0d47a1; margin-top: 0; padding-bottom: 5px; margin-bottom: 5px; }
        .subtitle { color: #757575; font-style: italic; margin-bottom: 20px; border-bottom: 2px solid #bbdefb; padding-bottom: 10px; }
        ...
        /* Certificate Actions links styled with theme color */
        .cert-link { color: #0d47a1; font-weight: bold; text-decoration: none; font-size: 14px; }
        .cert-link:hover { text-decoration: underline; color: #002171; }
        ...
        .btn-submit { background-color: #0d47a1; color: white; border: none; padding: 12px 25px; cursor: pointer; border-radius: 5px; font-weight: bold; font-size: 14px; text-transform: uppercase; }
...
<div class="header">
    ...
    <h1>Campaign Roster & Verification Summary</h1>
</div>

<div class="table-section">
    <h2><?php echo htmlspecialchars($event_name); ?></h2>
    <div class="subtitle">Campaign ID Reference: #<?php echo $event_id; ?> | Regional Registration Log</div>
    ...
    <?php if (empty($participants)): ?>
        <div class="no-data">No citizens have registered for this regional campaign yet.</div>
    <?php else: ?>
        <form method="POST" action="view_participants.php?event_id=<?php echo $event_id; ?>">
            <table>
                <thead>
                    <tr>
                        <th style="width: 15%;">Citizen ID</th>
                        <th style="width: 35%;">Full Name</th>
                        <th style="width: 20%;">Attendance (QR Status)</th>
                        <th style="width: 15%;">Certificate Action</th>
                        <th style="width: 15%;">
                            <input type="checkbox" id="select_all" onclick="toggleSelectAll(this)"> 
                            <label for="select_all" style="font-size: 12px; margin-left: 5px; cursor:pointer; font-weight: bold;">Select All</label>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($participants as $citizen): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($is_verified): ?>
                                    <span class="proof-badge proof-success">Verified</span>
                                <?php else: ?>
                                    <span class="proof-badge proof-null">Absent / Null</span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <?php if ($is_verified): ?>
            ...
            <div class="action-footer">
                <button type="submit" name="issue_certificates" class="btn-submit">Issue Selected Certificates</button>
            </div>
        </form>
    <?php endif; ?>
</div>
