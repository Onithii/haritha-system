...
// 1. Secure Access Check: Ensure user is logged in and is a Divisional Secretariat (Role 4)
...
// Official Sri Lankan Districts list for validation and form selection
$districts_list = [
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($event_title) || empty($event_date) || empty($start_time) || empty($end_time) || empty($district) || empty($location) || $required_volunteers <= 0) {
        $error_msg = "Please fill out all fields with valid information.";
    } elseif (!in_array($district, $districts_list)) {
        $error_msg = "Invalid district choice submitted.";
        ...
        if (empty($error_msg)) {
            ...
            $insert_query = "INSERT INTO volunteer_events (created_by, event_title, description, event_date, start_time, end_time, district, location, required_volunteers, event_image, verification_token, status) 
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'OPEN')";
            
            $stmt = mysqli_prepare($conn, $insert_query);
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "isssssssiss", $ds_id, $event_title, $description, $event_date, $start_time, $end_time, $district, $location, $required_volunteers, $image_path, $verification_token);
                
                if (mysqli_stmt_execute($stmt)) {
                    $success_msg = "Volunteer event successfully published and listed for regional action!";
                    $generated_qr_url = "https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=" . urlencode($verification_token);

                    $event_title = $description = $event_date = $start_time = $end_time = $district = $location = "";
                    $required_volunteers = "";
                } else {
...
// 3. Fetch existing events generated by this DS Officer
$events_list = [];
$list_query = "SELECT event_id, event_title, event_date, district, location, verification_token FROM volunteer_events WHERE created_by = ? ORDER BY event_date DESC";
$list_stmt = mysqli_prepare($conn, $list_query);
if ($list_stmt) {
    mysqli_stmt_bind_param($list_stmt, "i", $ds_id);
...
<head>
    <meta charset="UTF-8">
    <title>Post Divisional Volunteer Opportunity</title>
    <style>
        ...
        .back-btn:hover { background-color: white; color: #0d47a1; }
        ...
        .form-section h2, .manage-section h2 { color: #0d47a1; margin-top: 0; border-bottom: 2px solid #bbdefb; padding-bottom: 10px; }
        ...
        input[type="text"], input[type="date"], input[type="time"], input[type="number"], input[type="file"], textarea, select {
            padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 14px; font-family: inherit; box-sizing: border-box; width: 100%;
        }
        input:focus, textarea:focus, select:focus { border-color: #0d47a1; outline: none; box-shadow: 0 0 5px rgba(13, 71, 161, 0.2); }
        textarea { resize: vertical; min-height: 100px; }
        
        .btn-submit { background-color: #0d47a1; color: white; border: none; padding: 12px 20px; cursor: pointer; border-radius: 5px; font-weight: bold; font-size: 16px; width: 100%; margin-top: 10px; text-transform: uppercase; }
        .btn-submit:hover { background-color: #002171; }

        .qr-card { background: #f5f8ff; border: 2px dashed #0d47a1; border-radius: 8px; padding: 25px; margin-bottom: 25px; text-align: center; }
        .qr-card h3 { margin-top: 0; color: #002171; }
...
<div class="header">
    <a href="ds_dash.php" class="back-btn">← Return to Dashboard</a>
    <h1>Divisional Environmental Action Center</h1>
    <p>Coordinate Macro-Level Support & Manage Sub-Divisional Volunteer Campaigns</p>
</div>
...
<div class="form-section">
    <h2>Post New Divisional Volunteer Opportunity</h2>
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        <div class="form-group">
            ...
            <input type="text" id="event_title" name="event_title" placeholder="e.g., Division-Wide Tree Planting Initiative" value="<?php echo isset($event_title) ? htmlspecialchars($event_title) : ''; ?>" required>
        </div>

        <div class="form-group">
            <label for="description">Detailed Description & Instructions:</label>
            <textarea id="description" name="description" placeholder="Provide event blueprints, target sectors, safety protocol parameters, or coordinator details..." required><?php echo isset($description) ? htmlspecialchars($description) : ''; ?></textarea>
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="number" id="required_volunteers" name="required_volunteers" min="1" placeholder="Ex: 100" value="<?php echo isset($required_volunteers) ? htmlspecialchars($required_volunteers) : ''; ?>" required>
            </div>
        </div>
        ...
        <div class="form-group">
            <label for="district">Target Region / District:</label>
            <select id="district" name="district" required>
                <option value="">-- Choose Sri Lankan District --</option>
                <?php foreach ($districts_list as $dist): ?>
                    <option value="<?php echo $dist; ?>" <?php if(isset($district) && $district == $dist) echo 'selected'; ?>>
                        <?php echo $dist; ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="form-group">
            ...
            <input type="text" id="location" name="location" placeholder="e.g., Main Hall, Secretariat Complex Ground" value="<?php echo isset($location) ? htmlspecialchars($location) : ''; ?>" required>
        </div>
...
<div class="manage-section">
    ...
    <?php if (empty($events_list)): ?>
        <p style="color:#666; font-size:14px;">You haven't posted any divisional volunteer events yet.</p>
    <?php else: ?>
            ...
            <tbody>
                <?php foreach ($events_list as $row): ?>
                    <tr>
                        ...
                        <td><span style="background:#e3f2fd; color:#0d47a1; padding:3px 8px; border-radius:10px; font-size:12px; font-weight:bold;"><?php echo htmlspecialchars($row['district']); ?></span></td>
