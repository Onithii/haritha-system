...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'submit_proof') {
        ...
        if (empty($db_token)) {
            $alert_msg = "❌ This event does not have a verification token generated by the administrator.";
            $alert_class = "alert-error";
            ...
            if (!empty($found_other_title)) {
                $alert_msg = "❌ Mismatched Event! You scanned the verification key for '<strong>" . htmlspecialchars($found_other_title) . "</strong>', but you are on the submission page for '<strong>" . htmlspecialchars($event['event_title']) . "</strong>'. Please select the correct event from your dashboard.";
            } else {
                $alert_msg = "❌ Invalid QR Code string. Please scan the official code provided by event coordinators at the venue.";
            }
            $alert_class = "alert-error";
        }
    }
...
<head>
    ...
    <style>
        ...
        .manual-input-box { margin-top: 25px; padding-top: 20px; border-top: 1px dashed #b0bec5; }
        .text-input-field { width: 80%; max-width: 350px; padding: 10px; font-size: 15px; font-family: monospace; text-align: center; border: 2px solid #b0bec5; border-radius: 4px; }
        .btn-submit-text { background-color: #37474f; color: white; border: none; padding: 10px 20px; font-size: 15px; font-weight: bold; border-radius: 4px; cursor: pointer; margin-left: 5px; }
        .btn-submit-text:hover { background-color: #212121; }
    </style>
</head>
<body>
...
<div class="details-container">
    ...
    <div class="event-body">
        ...
        <?php else: ?>
            ...
            <?php if ($attendance_verified || $alert_class === 'alert-success'): ?>
            ...
            <?php else: ?>
                ...
                <div class="proof-section" id="scannerContainer">
                    ...
                    <div class="manual-input-box">
                        <p style="color:#37474f; font-size:14px; font-weight:bold; margin-bottom:8px;">⚠️ QR Image Missing or Distorted?</p>
                        <p style="color:#607d8b; font-size:13px; margin:0 0 12px 0;">Enter the hexadecimal string key manually below to authorize enrollment:</p>
                        <form method="POST" action="">
                            <input type="hidden" name="action" value="submit_proof">
                            <input type="text" name="qr_code_data" class="text-input-field" placeholder="Paste or type 32-character key..." required autocomplete="off">
